...
def split_data(x, y, splittype='timed', splitfrac=0.1, verbose=False):
    if splittype == 'rand':
        rs1 = model_selection.ShuffleSplit(len(x), test_size=splitfrac)
        ...
        raise TypeError("Split type ERROR in ml.py")
...
def print_only_top_features(clf, feat, treatnames, feat_choice, nfeat=5):
    ...
    if (n_classes == 1):
        ...
        print("\nTop features for treatment {}:".format(str(treatnames[1])))
        for i in topk1:
        ...
        print("\n\nTop features for treatment {}:".format(str(treatnames[0])))
        for i in topk0:
            print(feature_scores[i])
            if feat_choice == 'ads':
                feat.choose_by_index(i).display()
        ...
        for i in range(0, n_classes):
            ...
            print("Top features for treatment {}:".format(str(treatnames[i])))
            for j in topk:
                if feat_choice == 'ads':
...
def print_top_features(x, y, feat, treatnames, clf, feat_choice, nfeat=5,
                       blocked=1):  # prints top nfeat features from clf+some numbers
    ...
    if n_classes == 1:  # `~~~~~~~~~~
        topk1 = np.argsort(feature_scores)[::-1][:nfeat]  # `~~~~~~~~~~
        print("\nFeatures for treatment {}:".format(str(treatnames[1])))
        for i in topk1:
            if feat_choice == 'ads':
                ...
                print(feat[i])
        topk0 = np.argsort(feature_scores)[:nfeat]  # `~~~~~~~~~~
        print("\n\nFeatures for treatment {}:".format(str(treatnames[0])))
        for i in topk0:
            if feat_choice == 'ads':
                feat.choose_by_index(i).printStuff(feature_scores[i],  # `~~~~~~~~~~
                                                   [Atrain[i], Btrain[i], Atest[i], Btest[i], A[i], B[i]],
        ...
        for i in range(0, n_classes):
            topk = np.argsort(feature_scores[i])[::-1][:nfeat]  # `~~~~~~~~~~
            print("Features for treatment {}:".format(str(treatnames[i])))
            for j in topk:
                if feat_choice == 'ads':
                    feat.choose_by_index(j).display()
                elif feat_choice == 'words':
...
def crossVal_algo(k, algo, params, x, y, splittype, splitfrac, verbose=False):  # performs cross_validation
    if splittype == 'rand':
        rs2 = model_selection.ShuffleSplit(len(x), test_size=splitfrac)
    elif splittype == 'timed':
    ...
    for p in product(*par):
        ...
        if score > maximum:
            ...
            max_params = p
            classifier = clf
    return maximum, max_params, classifier
