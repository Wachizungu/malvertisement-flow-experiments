...
class AdVector:
    ...
    def display(self, choice):
        ...
        i = 0
        chunks = re.split(r'\+', choice)
        for ad in self.data:
            i += 1
            sys.stdout.write("{} ".format(i))
            if 'url' in chunks:
                sys.stdout.write("{} ".format(ad.url))
            if 'title' in chunks:
                sys.stdout.write("{} ".format(ad.title))
            if 'body' in chunks:
                sys.stdout.write("{} ".format(ad.body))
            if 'cat' in chunks:
                sys.stdout.write("{} ".format(ad.cat))
            if 'time' in chunks:
                sys.stdout.write("{} ".format(ad.time))
            if 'label' in chunks:
                sys.stdout.write("{} ".format(ad.label))
            print("")

    def choose_by_index(self, index):
        return self.data[index]

    def count_labels(self, lbl):
        c1 = 0
        for ad in self.data:
            if ad.label == lbl:
                c1 += 1
    ...
    def gen_word_vec(self, word_v,
                     wchoice=W_CHOICE):  # check generates a vector of words from AdVector, fits it to word_v
        vec = []
