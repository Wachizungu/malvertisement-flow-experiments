...
class NewsVector:
    ...
    def display(self, choice):
        ...
        for news in self.data:
            ...
            if 'agency' in chunks:
                sys.stdout.write("{} ".format(news.agency))
            if 'title' in chunks:
                sys.stdout.write("{} ".format(news.title))
            # if('body' in chunks):
            #    sys.stdout.write("{} ".format(news.body))
            if 'ago' in chunks:
                sys.stdout.write("{} ".format(news.ago))
            if 'time' in chunks:
                sys.stdout.write("{} ".format(news.time))
            if 'label' in chunks:
                sys.stdout.write("{} ".format(news.label))
            print("")
    ...
    def news_weight(self, news, wchoice):
        ...
        if wchoice == NUM:
            ...
            raise ValueError("Illegal W_CHOICE")

    def gen_word_vec(self, word_v,
                     wchoice=W_CHOICE):  # check generates a vector of words from NewsVector, fits it to word_v
        vec = []
    ...
    def gen_news_vec(self, newsv,
                     choice=CHOICE):  # check self is news_union  # generates a vector of newss from NewsVector
        vec = [0] * self.size()
        for news in self.data:
...
def news_sim(newsv1, newsv2):  # check
    if SIM_CHOICE == JACCARD:
        ...
        raise ValueError("Illegal SIM_CHOICE")
