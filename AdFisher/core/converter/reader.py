...
def ad_vectors(ad_list, filtered_by=None):  # returns a frequency vector of ads, when input a list of adVecs
    ad_union = adVector.AdVector()
...
def freq_news_vectors(ad_list):  # returns a frequency vector of news, when input a list of newsVecs
    news_union = news.NewsVector()
    for newsv in ad_list:
...
def interest_vectors(ad_list):  # returns a frequency vector of interests, when input a list of interessts
...
def get_interest_vectors(advdicts):
    ...
    for advdict in advdicts:
        interest_list.extend(advdict['interests'])
    X, labels, feat = interest_vectors(interest_list)
    y = []
    if labels[0] == '':
        for advdict in advdicts:
...
def get_feature_vectors(advdicts, feat_choice, filtered_by=None):  # returns observation vector from a list of rounds
    ...
    if feat_choice == 'words':
        for advdict in advdicts:
            temp_list.extend(advdict['advector'])
        x, labels, feat = word_vectors(temp_list)
    elif feat_choice == 'ads':
        for advdict in advdicts:
            temp_list.extend(advdict['advector'])
        x, labels, feat = ad_vectors(temp_list, filtered_by)
    elif feat_choice == 'news':
        for advdict in advdicts:
            temp_list.extend(advdict['newsvector'])
        x, labels, feat = freq_news_vectors(temp_list)
    ...
    for line in fo:
        ...
        if linetype == 'meta':
            ...
            #               print block_id
                ...
                assignment = [int(x) for x in re.split(r'@\|', value)]
...
def read_old_log(log_file):
    ...
    for line in fo:
        ...
        if chunks[0] == gmarker and r == 0:
            ...
            identifier = int(chunks[3])
            loadtimes[identifier] += delta
        elif chunks[1] == 'reload':
            identifier = int(chunks[2])
            reloads[identifier] += 1
        elif chunks[1] == 'errorcollecting':
            identifier = int(chunks[2])
            errors[identifier] += 1
        elif chunks[1] == 'prepref':
            identifier = int(chunks[4])
            ints[identifier].remove_interest()
        elif chunks[1] == 'pref':
            identifier = int(chunks[4])
            int_str = chunks[3]
            ints[identifier].set_from_string(int_str)
        elif chunks[0] == 'news':
            ind_news = news.News({'Time': datetime.strptime(chunks[3], "%Y-%m-%d %H:%M:%S.%f"), 'Title': chunks[4],
                                  'Agency': chunks[5], 'Ago': chunks[6], 'Body': chunks[7].rstrip(),
                                  'Label': chunks[2]})
