...
class GoogleNewsUnit(google_ads.GoogleAdsUnit):
    ...
    def get_allbutsuggested(self):  # Slow execution
        ...
        sys.stdout.flush()
        for td in tds:
            title = td.find_element_by_xpath(".//div[@class='esc-lead-article-title-wrapper']/h2/a/span").get_attribute(
                'innerHTML')
            ...
            try:
                heading = td.find_element_by_xpath(
                    "../../../../../../../../../div[@class='section-header']/div/div/h2/a/span").get_attribute(
                    'innerHTML')
            except:
                pass
            if "Suggested" in heading:
                ...
                continue
            #           print "entering"
            news = strip_tags(tim + "@|" + heading + "@|" + title + "@|" + agency + "@|" + ago + "@|" + body).encode(
                "utf8")
            self.log('measurement', 'news', news)

    def get_news(self, news_type, reloads, delay):
        ...
        while rel < reloads:  # number of reloads on sites to capture all ads
            ...
            try:
                ...
                if news_type == 'top':
                    ...
                    print("No such news category found: {}!".format(news_type))
                e = datetime.now()
