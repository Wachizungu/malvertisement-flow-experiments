...
def random_user_email(first_name):
    num_chars_in_last_name = random.randrange(5, 40)
    last_name = get_random_string(num_chars_in_last_name)
    return first_name + '@' + last_name + '.com'
...
class IndeedAdsUnit(browser_unit.BrowserUnit):
    ...
    def create_user(self, gender, job_description):
        ...
        first_name = ''
        if gender == 'male':
            first_name = random_male_first_name()
        else:
            first_name = random_female_first_name()
        email_txt = random_user_email(first_name)
