...
def load_ads_from_json(log_name,session):
    ...
    ----------
    log_name : str
        path to log file in which we looked for sessions. json files should be in same directory
    session : str

    Returns
    -------
    ad_lines : list of Ad
        list of the ads found in the json array
...
def find_json_logs(log_file):
    ...
    Parameters
    ----------
    log_file: input file, which will be parsed to find session id

    Returns: list containing the id's of the sessions
    -------
...
def main(log_file):
    ...
    for _, ad in all_data:
        urls.add(ad.url)
    with open('urls_jeroenpinoy_website.txt', 'wb') as csvfile:
        fieldnames = ['url']
